...
export default class BillingForm extends React.Component {
	...
	onSubmit(e) {
  	...
  	componentDidMount() {
		this.billingData = JSON.parse(localStorage.getItem('billingData'));

		//if user data already stored, read in and set from storage
		if (localStorage.getItem('billingData')) {
			this.setState({
				inputState: this.billingData.inputState,
				first: this.billingData.first,
				last: this.billingData.last,
				aptNumber: this.billingData.aptNumber,
				streetName: this.billingData.streetName,
	...
	componentWillUpdate(nextProps, nextState) {
		localStorage.setItem('billingData', JSON.stringify(nextState));
	}
		...
		if(this.state.inputState === "NEUTRAL") {
			...
			return (
				<div className="user-data">
					...
					<button onClick={() => this.updateInputState("NEUTRAL")}>Edit Billing Address</button>
				</div>
			)
		}
		else {
